...
const Dashboard = () => {
  ...
  useEffect(() => {
    if (user && user.$id) {
      ...
      appwriteService.getPostsByUser(user.$id)
        .then((posts) => {
          if (posts) {
      ...
      <div className="flex flex-col py-20 gap-y-10 Dashboard max-md:items-center max-md:py-10">
        ...
        <div className="flex gap-10 max-md:justify-center max-md:flex-col max-md:items-center max-md:gap-5 max-md:w-[400px] max-md:px-5">
          <div className="flex p-6 bg-gray-100 shadow rounded-xl gap-x-10 dark:bg-zinc-900">
            ...
            <div className="">
              <img src={userIcon} className="w-[200px]" alt="User Icon" />
            </div>
          </div>
          <div className="flex p-6 bg-gray-100 shadow rounded-xl gap-x-10 dark:bg-zinc-900">
            <div>
            ...
            <div>
              <img src={post} className="w-[200px]" alt="Post Icon" />
            </div>
          </div>
          ...
          <div className="flex flex-col gap-5 ">
            {posts.length ? (
              posts.map((post) => (
                <div
                  key={post.$id}
                  className="flex items-center justify-between w-full p-4 bg-gray-200 shadow rounded-xl dark:bg-zinc-800 max-md:flex-col max-md:gap-5"
                >
                  <div className="flex items-center max-md:flex-col max-md:justify-center">
                    <img
                      src={appwriteService.getFilePreview(post.featuredImage)}
                      className="w-32 mr-4 rounded-md max-md:w-full max-md:mr-0 max-md:mb-3"
                      alt="Post Image"
                    />
                    <div className="">
